{{ config(materialized='incremental',
    unique_key='unique_incremental_key',
        ...
        after_commit("create index if not exists {{ this.name }}__index_on_date_trunc_order_week on {{ this }} (date_trunc('week',order_date AT TIME ZONE 'UTC'))")
      ]
...
SELECT
concat(order_id,line_item_id) as unique_incremental_key,
order_id,
line_item_id,
source,
unit_price,
...
REGEXP_REPLACE(billing_email, '^\s*(.*?)\s*$', '\1') AS billing_email,
REGEXP_REPLACE(billing_first_name, '^\s*(.*?)\s*$', '\1') AS billing_first_name,
REGEXP_REPLACE(billing_last_name, '^\s*(.*?)\s*$', '\1') AS billing_last_name,
REGEXP_REPLACE(shipping_address1, '^\s*(.*?)\s*$', '\1') AS shipping_address1,
REGEXP_REPLACE(shipping_address2, '^\s*(.*?)\s*$', '\1') AS shipping_address2,
REGEXP_REPLACE(shipping_city, '^\s*(.*?)\s*$', '\1') AS shipping_city,
REGEXP_REPLACE(shipping_state, '^\s*(.*?)\s*$', '\1') AS shipping_state,
REGEXP_REPLACE(shipping_postalcode, '^\s*(.*?)\s*$', '\1') AS shipping_postalcode,
REGEXP_REPLACE(shipping_country, '^\s*(.*?)\s*$', '\1') AS shipping_country,
REGEXP_REPLACE(billing_address1, '^\s*(.*?)\s*$', '\1') AS billing_address1,
REGEXP_REPLACE(billing_address2, '^\s*(.*?)\s*$', '\1') AS billing_address2,
REGEXP_REPLACE(billing_address3, '^\s*(.*?)\s*$', '\1') AS billing_address3,
REGEXP_REPLACE(billing_city, '^\s*(.*?)\s*$', '\1') AS billing_city,
REGEXP_REPLACE(billing_country, '^\s*(.*?)\s*$', '\1') AS billing_country,
REGEXP_REPLACE(billing_postalcode, '^\s*(.*?)\s*$', '\1') AS billing_postalcode,
REGEXP_REPLACE(shipping_phone_number, '^\s*(.*?)\s*$', '\1') AS shipping_phone_number,
return_order_id,
...
WHERE source <> 'Planner'
      AND not (order_location_type = 'Internally Drafted' 
      AND gross_revenue = 0::numeric)
